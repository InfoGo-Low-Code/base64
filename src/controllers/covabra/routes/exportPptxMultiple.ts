...
function chunkMetricas(metricas: any[], size = 2) {
  const pages = []

  for (let i = 0; i < metricas.length; i += size) {
    pages.push(metricas.slice(i, i + size))
  }
...
function generateRandomColor(): string {
  // gera cores mais "sóbrias", evita muito claro
...
function buildChartColors(
  ...
  while (colors.length < totalMetricas) {
...
export function exportPptxMultiple(app: FastifyZodTypedInstance) {
  app.post(
    '/covabra/exportPptxMultiple',
    {
      schema: {
        body: z.object({
          titulo: z.string(),
          ...
          metricas: z.array(
            z.object({
              ...
              tipo: z.enum(['bar', 'line']).optional(),
              cor: z.string().optional(),
              eixoSecundario: z.boolean().optional(),
            })
          ).min(1).optional(),
          ...
          metricasTable: z.array( z.record(z.union([z.string(), z.number(), z.null()])) ).optional(),
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (tipoGrafico === 'bar-line') {
          const paginas = chunkMetricas(metricas!)

          paginas.forEach((paginaMetricas) => {
            const slide = pptx.addSlide()

            const barras = paginaMetricas!.filter(m => m.tipo !== 'line')
            const linhas = paginaMetricas!.filter(m => m.tipo === 'line')
